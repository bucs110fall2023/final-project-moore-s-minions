...
#         elif x_position == idx * (SCREEN_WIDTH // NUM_RECTANGLES):
#             # Mark the rectangles at the right side as green
#             pygame.draw.rect(screen, FINAL_COLOR, (x_position, y_position, RECTANGLE_WIDTH, heights[idx]))
#         else:
...
# Bubble Sort algorithm
def bubble_sort(heights, sorted_indices):
    n = len(heights)
    for i in range(n):
        for j in range(0, n-i-1):
            if heights[j] > heights[j+1]:
                ...
                heights[j], heights[j+1] = heights[j+1], heights[j]
                # Update display
                update_display(j, j+1, heights, sorted_indices)

# Function to update the display
def update_display(i, j, heights, sorted_indices):
    screen.fill(BLACK)
    for idx in range(NUM_RECTANGLES):
        x_position = idx * (SCREEN_WIDTH // NUM_RECTANGLES)
        y_position = SCREEN_HEIGHT - heights[idx]
        ...
        if idx == i or idx == j:
            ...
            # Mark the rectangles in their final sorted positions as green
            pygame.draw.rect(screen, FINAL_COLOR, (x_position, y_position, RECTANGLE_WIDTH, heights[idx]))
        else:
            # Regular blue color for other rectangles
            pygame.draw.rect(screen, RECTANGLE_COLOR, (x_position, y_position, RECTANGLE_WIDTH, heights[idx]))
...
# Main loop
running = True
sorted_indices = set()  # Track indices that are in their final sorted positions
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
    ...
    # Run the Bubble Sort algorithm
    bubble_sort(rectangle_heights, sorted_indices)

    # Check for indices in their final sorted positions
    for idx in range(NUM_RECTANGLES):
        if idx not in sorted_indices and rectangle_heights[idx] == SCREEN_HEIGHT - idx * (RECTANGLE_HEIGHT_MAX // NUM_RECTANGLES):
            sorted_indices.add(idx)

    # Quit the loop after the sorting is done
    if len(sorted_indices) == NUM_RECTANGLES:
        running = False
